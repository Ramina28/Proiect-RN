...
|----------|------------------------------------------|--------------|--------------|-------------------|----------------|
| Baseline | Arhitectură simplă, Fără Augmentare, LR=0.01 | 0.65 | 0.62 | 20 min | Overfitting rapid; diferență mare între Train și Validation.. |
| Exp 1 | Adăugare Dropout (0.3) în straturile Dense | 0.69 | 0.66 | 40 min | Reducere overfitting și stabilizare parțială a validării. |
| Exp 2 | Scădere Learning Rate (0.01 → 0.001) | 0.72 | 0.70 | 60 min | Convergență mult mai stabilă, grafic loss mai lin. |
| Exp 3 | Batch Size 32 → 16 | 0.75 | 0.72 | 35 min | Generalizare mai bună pe setul de validare.|
| Exp 4 | Dropout 0.3 → 0.5 | 0.73 | 0.69 | 90 min | Regularizare mai puternică, dar ușoară scădere a performanțe |
| Exp 5 | + Data Augmentation (Zoom, Brightness) | 0.87 | 0.87 | 90 min | **BEST** Model final – creștere majoră a robusteții și a capacității de generalizare. |

**Justificare alegere configurație finală:**
```
Configurația finală corespunde Exp 5, care include Data Augmentation și a generat modelul final optimized_model.h5.

1. Performanță superioară pe test set
Modelul final a obținut:
Accuracy: 87.70%
F1-score (macro): 87.70%
pe un set de test de 1228 imagini, depășind semnificativ modelul baseline (65%).

2. Augmentările de tip brightness și zoom au redus dependența modelului de condițiile de iluminare și de poziționarea subiectului. Modelul a fost forțat să învețe caracteristicile relevante ale leziunilor (textură și distribuție), nu doar informații de culoare.

3. Reducerea overfitting-ului
Îmbunătățirea progresivă a performanței a fost obținută prin combinarea:
Learning rate redus (Exp 2)
Batch size mai mic (Exp 3)
Regularizare prin Dropout (Exp 1)
Această combinație a redus diferența dintre performanța pe train și validation și a dus la o generalizare mai bună.

4. Cost computațional acceptabil
Timpul de antrenare (~50 minute) este rezonabil pentru reantrenări ulterioare și nu introduce un cost computațional ridicat.
```
...
|----------------|-------------------|------------------------|-----------------|
| **Model încărcat** | `trained_model.h5` (baseline) | `optimized_model.h5` | Modelul final obține Accuracy = 87.7% și F1-score = 87.7%, oferind o generalizare semnificativ mai bună față de baseline. |
...
1. **Model înlocuit:** `models/trained_model.h5` → `models/optimized_model.h5`
   - **Îmbunătățire:** Accuracy **+22.7%** (de la 0.65 la 0.87.7), F1-score **+25.7%** (de la 0.62 la 0.87.7).
...
**Clasa cu cea mai bună performanță:** **Acnee**
- **Precision:** 88.0%
- **Recall:** 87.9%
- **Explicație:** Modelul identifică corect majoritatea cazurilor de acnee și prezintă un echilibru bun între precision și recall.

**Clasa cu cea mai slabă performanță:** **Eczeme**
- **Precision:** 87.4%
- **Recall:** 87.5%
- **Explicație:** Performanța este similară cu cea pentru acnee, dar există o ușoară tendință de confuzie în cazurile în care leziunile au caracteristici vizuale comune (roșeață difuză sau inflamație extinsă).

**Confuzii principale:**

1. **Clasa [Acnee] confundată cu clasa [Eczeme] în 12.1% din cazuri** (76 imagini)
...
2. **Clasa [Eczeme] confundată cu clasa [Acnee] în 12.5% din cazuri** (75 imagini)
...
**Abordare:** Optimizarea a fost realizată printr-o strategie de căutare manuală iterativă (Manual Search), bazată pe rulări succesive și pe analiza curbelor de învățare (loss și accuracy pe train și validation). de analiza curbelor de învățare (Loss/Accuracy curves).
După fiecare experiment, au fost analizate:
- evoluția train_loss vs val_loss
- stabilitatea val_accuracy
- semne de overfitting sau subantrenare
Pe baza acestor observații, hiperparametrii au fost ajustați progresiv.
...
|-------------|-------------|-------------|-------------|----------------------|------------|
| **Accuracy** | ~33% (Random) | 65% | **87.7%** | ≥85% | Atins |
| **F1-score (macro)** | ~0.30 | 0.62 | **0.87.7** | ≥0.80 | Depășit |
| **Precision** | ~0.33 | 0.65 | **0.87** | ≥0.85 | Atins |
| **Recall** | ~0.33 | 0.60 | **0.87** | ≥0.85 | Atins |
| **False Negative Rate**| N/A | ~40% | **~12%** | ≤10% | Aproape |
...
- [ ] Modul de Telemedicină (Human-in-the-Loop): Implementarea unei funcționalități pentru trimiterea securizată (criptată) a imaginii și a predicției AI către un medic dermatolog partener pentru validare finală. Aceasta ar transforma aplicația într-un instrument de triaj medical.
```
